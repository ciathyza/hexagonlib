...
package com.hexagonstar.util.potrace
{
	...
	public class POTrace
	{
							...
							for (k = 0; k < curves.length; k++)
							{
								...
								switch(curve.kind)
								{
									case CurveKind.BEZIER:
										backend.addBezier(curve.a.clone(), curve.cpa.clone(), curve.cpb.clone(), curve.b.clone());
										break;
									case CurveKind.LINE:
										backend.addLine(curve.a.clone(), curve.b.clone());
										break;
								}
							}
						}
		...
		private function bestpolygon(path:Path):void
		{
			...
			var k:int;
			var n:int = path.pt.length;
			var pen:Vector.<Number> = new Vector.<Number>(n + 1);
			// penalty vector
			var prev:Vector.<int> = new Vector.<int>(n + 1);
			// best path pointer vector
			var clip0:Vector.<int> = new Vector.<int>(n);
			// longest segment pointer, non-cyclic
			var clip1:Vector.<int> = new Vector.<int>(n + 1);
			// backwards segment pointer, non-cyclic
			var seg0:Vector.<int> = new Vector.<int>(n + 1);
			// forward segment bounds, m <= n
			var seg1:Vector.<int> = new Vector.<int>(n + 1);
			// backward segment bounds, m <= n
		...
		private function adjust_vertices(path:Path):void
		{
			...
			for (i = 0; i < m; i++)
			{
				...
				var det:Number;
				var min:Number;
				// minimum for minimum of quad. form
				var cand:Number;
				// candidate for minimum of quad. form
				var xmin:Number;
		...
		private function opti_penalty(path:Path, i:int, j:int, res:Opti, optTolerance:Number, convc:Vector.<int>, areac:Vector.<Number>):Boolean
		{
			...
			var R:Number = area / A;
			// relative area
			var alpha:Number = 2 - Math.sqrt(4 - R / 0.3);
		...
		private function pointslope(path:Path, i:int, j:int, ctr:Point, dir:Point):void
		{
			...
			var lambda2:Number = (a + c + Math.sqrt((a - c) * (a - c) + 4 * b * b)) / 2;
			// larger e.value
